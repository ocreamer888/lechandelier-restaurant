...
export default function Menu3({ data, locale = 'es' }: Menu3Props = {}) {
  ...
  const getCategoryLabel = (cat: Category) => {
    switch (cat) {
      case "Entradas": return t('starters');
      case "Platos Fuertes": return t('mains');
      case "Postres": return t('desserts');
      default: return cat;
    }
  };
  ...
  return (
    <section className="bg-gradient-to-b from-transparent via-black/20 to-transparent py-16 md:py-24 text-white" id="menu">
      <div className="mx-auto w-full max-w-5xl px-4 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center justify-center gap-2">
          {(["Entradas", "Platos Fuertes", "Postres"] as Category[]).map((c) => (
            <button
              key={c}
              onClick={() => setActive(c)}
              className={`rounded-full border px-4 py-2 text-sm transition ${active === c
        ...
        <SectionOrnament text={getCategoryLabel(active)} />

        <ul className="space-y-8">
          {items.map((it) => {
            const mainName = locale === 'en' ? it.nameEnglish : it.name;
            const subName = locale === 'en' ? it.name : it.nameEnglish;
            ...
            return (
              <li key={it.id} className="grid grid-cols-[250px_1fr_auto] md:grid-cols-[400px_1fr_auto] lg:grid-cols-[700px_1fr_auto] items-start gap-5">
                ...
                <div className="col-span-1">
                  ...
                  <p className="mt-1 text-xs italic text-white/50">{subName}</p>
